...
<ul>
  ...
  <li><a href="#illustrative-debug-objects">14. Illustrative Debug Objects</a></li>
...
<ul>
  ...
  <li><code>IDE/Execution observability.md</code> — source-aligned observability projection consumed by debugging,</li>
  ...
  <li><code>Expression/Control structures.md</code> — canonical source representation of structures and regions,</li>
  <li><code>Expression/State and cycles.md</code> — canonical source representation of local memory and cycle constraints,</li>
  <li><code>Expression/Widget interaction.md</code> — widget-related execution objects and explicit UI sequencing,</li>
  <li><code>Language/Execution model.md</code> — live execution instance, activation, and execution context,</li>
  <li><code>Language/Execution control and observation boundaries.md</code> — safe observation points, pause-consistent snapshots, safe debug stops, and terminal boundary semantics,</li>
  ...
  <li><code>Libraries/UI.md</code> — primitive-local UI interaction behavior.</li>
</ul>
...
<h3>4.5 Capability profiles and debugging support levels remain distinct</h3>
...
but they are not part of the minimum baseline required by Section 13.
</p>
...
<p>
This is the dataflow analogue of breaking on value arrival.
</p>
...
<h3>7.9 Multiple simultaneous matches</h3>
...
<ul>
  <li>for an ordinary node, it behaves as the next debugger-relevant stop on the reachable path,</li>
  <li>for a <code>subfrog</code> call, it MAY enter the callee scope,</li>
  <li>for a structure, it MAY enter the selected dynamic region or the first debuggable activity within it.</li>
</ul>
...
<ul>
  <li>for an ordinary primitive activation, <code>step_over</code> MAY behave like <code>step_into</code>,</li>
  <li>for a <code>subfrog</code> call, it runs through the nested callee activity and returns to the caller scope,</li>
  <li>for a structure activation, it runs through the selected dynamic structure activity and returns to the parent scope.</li>
</ul>
...
<p>
Inactive regions MUST NOT produce stepping stops for that structure activation.
</p>

<h3>9.4 Loop structures</h3>
...
<ul>
  <li><code>step_into</code> from loop entry MAY stop at the first eligible stop of the first executed iteration,</li>
  <li><code>step_over</code> from loop entry MUST run through that loop activation and return after loop completion,</li>
  <li><code>step_out</code> from inside the loop body MUST continue until the current loop activation completes and then return to the parent scope.</li>
</ul>
...
<ul>
  <li><code>step_into</code> from loop entry MAY stop at the first eligible stop of the first executed iteration,</li>
  <li><code>step_over</code> from loop entry MUST run until that loop activation terminates normally, faults, or is aborted,</li>
  <li><code>step_out</code> from inside the loop body MUST continue until the current loop activation terminates and then return to the parent scope.</li>
</ul>
...
<h3>9.5 Local memory inside nested scopes</h3>

<p>
When local-memory primitives such as <code>frog.core.delay</code> are active inside nested scopes,
...
<p>
If readiness is exposed by the active observability support level,
the IDE SHOULD also be able to highlight readiness-sensitive states.
</p>
...
<h3>10.4 No mandatory rendering style</h3>
...
<h3>12.1 Widget-related execution objects</h3>
...
<h3>12.2 Front-panel reflection</h3>
...
<h3>13.1 Minimum interactive debugging baseline</h3>
...
<h3>13.2 Optional standardized additions</h3>
...
<h2 id="illustrative-debug-objects">14. Illustrative Debug Objects</h2>
...
<h3>14.3 Paused debug snapshot</h3>
...
<h3>14.4 Fault-directed pause</h3>
