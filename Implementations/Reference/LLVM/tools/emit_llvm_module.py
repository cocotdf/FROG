...
MODULE_TEMPLATE = """; FROG example 05 - emitted LLVM module
...
def main() -> None:
    root = Path(__file__).resolve().parent.parent / "examples" / "05_bounded_ui_accumulator"
    lowering_path = root / "main.lowering.json"
    module_path = root / "module.ll"

    lowering = json.loads(lowering_path.read_text(encoding="utf-8"))
    iterations = lowering["algorithm"]["iterations"]

    module = MODULE_TEMPLATE.format(iterations=iterations)
