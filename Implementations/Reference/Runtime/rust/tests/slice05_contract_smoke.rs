...
fn published_slice05_contract_deserializes_and_exposes_expected_shape() {
    ...
    assert_eq!(contract.artifact_version, "0.1");
    ...
    assert_eq!(contract.source_ref.spec_version, "0.1");
    ...
    assert!(unit
        ...
        .widgets
        .iter()
        .any(|w| w.widget_id == "ctrl_input" && w.binding.public_input_id.as_deref() == Some("input_value")));
    assert!(unit
        .ui_binding
        .widgets
        .iter()
        .any(|w| w.widget_id == "ind_result" && w.binding.public_output_id.as_deref() == Some("result")));

    assert_eq!(unit.property_writes.len(), 2);
    assert!(unit
        .property_writes
        .iter()
        .all(|w| w.operation == "frog.ui.property_write" && w.member == "face_color"));
}
