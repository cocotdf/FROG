...
EXPECTED_RESULT = 6.0
TEST_INPUTS = {"ctrl_a": 2.25, "ctrl_b": 3.75}
...
def example_source_path() -> Path:
    return repo_root_from_here() / "Examples" / "02_ui_value_roundtrip" / "main.frog"
...
def check_validate() -> None:
    ...
    assert_equal(subset["public_interface"], False, "validate.validated_subset.public_interface")
    assert_equal(subset["widget_value"], True, "validate.validated_subset.widget_value")
...
def check_emit_contract() -> None:
    ...
    assert_equal(data["assumptions"]["ui_binding"]["kind"], "natural_widget_value", "contract.assumptions.ui_binding.kind")
    assert_equal(len(data["units"]), 1, "contract.unit_count")

    unit = data["units"][0]
    assert_equal(unit["id"], "main", "contract.unit.id")
    assert_equal(len(unit["boundaries"]), 0, "contract.public_boundary_count")
    assert_equal(len(unit["ui_bindings"]["inputs"]), 2, "contract.ui_input_count")
    assert_equal(len(unit["ui_bindings"]["outputs"]), 1, "contract.ui_output_count")
...
def check_run() -> None:
    ...
    result_value = float(runtime["outputs"]["ui"]["ind_result"])
    assert_close(result_value, EXPECTED_RESULT, "run.runtime.outputs.ui.ind_result")
...
def main() -> int:
    ...
    check_emit_contract()
    check_run()
    print("02_ui_value_roundtrip regression: PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"02_ui_value_roundtrip regression: FAIL\n{exc}", file=sys.stderr)
        raise SystemExit(1)
